...
**Needs You**:
"该 session 正在等用户交互"的布尔标志。判定宁缺毋滥：只有能唯一识别并配对的直接 Session 信号才点亮；不确定时不点亮，绝不把 Idle 错报成 Waiting。语义工具事件缺少 `tool_use.id`（工具调用标识）时无法可靠判断后续是否已有对应结果，按标准状态折叠为 Working，不点亮 Needs You。已有等待只能被相同 id 的 `tool_result`（工具结果）结束；不匹配结果不得改变 Waiting、等待原因、Needs You 或 Elapsed。（ADR-0006）
