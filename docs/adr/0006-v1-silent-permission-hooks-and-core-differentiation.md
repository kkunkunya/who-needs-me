...
用户在本会话明确拍板：**给 Claude 和 Codex 各装一个 hook 是可接受的（"很多类型的产品都走这样的 hook，只要不乱注入提示词都没问题"）。**

## 决策

1. **V1 采集为 Claude 与 Codex 都接入一个沉默权限 hook**——Claude 用 `PreToolUse` / `Notification`，Codex 用 `PermissionRequest`（`~/.codex/hooks.json`）。两者都遵守 ADR-0002 的"沉默 hook"硬约束（只追加写本地事件、`exit 0`、不向 Agent 上下文输出、不返回审批决策、不干预原生审批流）。这**修订** ADR-0002 与 PRD（Issue #2）里"V1 沉默 hook 只接 Claude Code / Codex 无 hook 纯文件"以及"Codex hooks 仍在快速变化"的旧判断——Codex hook 主干（含 `PermissionRequest`）已稳、可依赖，边缘字段仍在扩展。
2. **把 hook 从"可选增强"重新定级为 V1 核心交付物**：它是"权限/审批语义 Needs-You"这条差异化的使能器，也是 WhoNeedsMe 相对头号竞品 Agent Island 真正能赢的地方。**零安装文件底座仍然保留**（不装 hook 也能看到会话列表、四态、以及"等回答问题 / 等计划确认"两类 Waiting）；只有"权限/审批类 Needs-You"的点亮**绑定到装 hook**（不装则该类降级为不点亮，守住"宁缺毋滥"，不用超时启发式猜——这解掉了本会话的决策 A/C）。
...
- hooks 安装器（ADR-0002 已列为必备件）的范围从"仅 Claude"扩为"Claude + Codex 各一份沉默 hook 的一键安装/检查/修复"。
