...
- Tray：calm / working / attention 三张静态 frame，原子替换，无动画文件。
  **来源:** Phase 5 自定（用户已确认）。
- Panel Header：aggregate state change 播放一次 `200ms`，只动 `eyes / ears / head / body / tail / accent` 中必要 groups。
  **来源:** Phase 5 自定（用户确认范围；easing 为 `cubic-bezier(0.2, 0.8, 0.2, 1)`）。
- Alert：出现时一次抬耳/睁眼/轻微探头；不循环；Alert 内容与 CTA 不随猫位移。
  **来源:** Phase 5 自定（用户已确认范围）。
...
|---|---|---|---|---|
| micro | Tray | calm / working / attention | vector master | calm/working template-safe PNG；attention coral non-template PNG |
| compact | Panel Header | calm / working / attention | layered SVG | inline SVG |
| compact | Alert Card | attention | same compact character | inline SVG |
| illustration | Empty State | sleeping calm | SVG | inline SVG |

### 10.3 SVG 分层

最少稳定 groups：`eyes`、`ears`、`head`、`body`、`tail`、`accent`。未参与动效的路径仍保留语义 group，避免实现阶段重新拆图。

### 10.4 开工前证据

1. 人审通过的 character sheet：`artifacts/design-qa/cat-assets/who-needs-me-cat-character-sheet-v1.png`（2026-07-17，Kun approved）。
2. 分层 SVG masters。
3. Tray 普通/高密度显示 scale 实机截图。
4. 无标签 state contact sheet。
5. Panel state change / Alert entrance 的 start/end frame、duration、easing、reduced-motion note。
...
- calm / working template-safe monochrome；attention coral。
