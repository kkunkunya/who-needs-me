...
| Tray Icon | 独立 surface | 常驻表达聚合状态 | `micro cat glyph` | calm / working / attention 三张静态全色栅格状态图（PNG，非 template 单色） | calm / working / attention @1x/@2x |
...
- **Cat runtime assets 走已批准的 gpt-image-2 栅格例外**（2026-07-17 Kun 拍板）：以人审通过的 character sheet 为锚，用 gpt-image-2 生成栅格母版，经 `/produce-assets` 抠透明底 + resize 交付透明 PNG/WebP @1x/@2x。**解除 layered SVG master 与 Tray template 单色要求**；Tray 改为全色栅格（Kun 已接受浅色菜单栏重着色风险）。**人审门禁保留**：character sheet、栅格母版、Tray 实机截图与 motion note 未经人审前，不得把临时 raster 概念图当运行时资产。
